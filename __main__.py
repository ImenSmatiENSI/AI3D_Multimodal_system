...
def isnetworkfile(fname):
    ...
    if ext not in ['.pyt', '.meta', '.tf','.onnx', '.pb','.h5']:
        raise argparse.ArgumentTypeError('only .pyt, .tf, .onnx, .pb, .h5 and .meta formats supported')
    return fname
...
def normalize(image, means, stds, dataset):
    ...
    if len(means) == len(image):
        for i in range(len(image)):
            ...
            if stds!=None:
                image[i] /= stds[i]
    elif dataset == 'mnist'  or dataset == 'fashion':#or our dataset
        for i in range(len(image)):
...
def normalize_poly(num_params, lexpr_cst, lexpr_weights, lexpr_dim, uexpr_cst, uexpr_weights, uexpr_dim, means, stds, dataset):
    # normalization taken out of the network
    if dataset == 'mnist' or dataset == 'fashion':# or our datset
...
def get_tests(dataset, geometric):
    if geometric:
        ...
        if config.subset == None:
            ...
            print("Samples are available and loaded")
...
parser.add_argument('--netname', type=isnetworkfile, default=config.netname, help='the network name, the extension can be only .pb, .pyt, .tf, .meta, .h5 and .onnx')
...
is_saved_keras_model = file_extension==".h5"
...
assert is_trained_with_pytorch or is_saved_tf_model or is_pb_file or is_tensorflow or is_onnx or is_saved_keras_model , "file extension not supported"
...
non_layer_operation_types = ['AssignVariableOp','NoOp', 'Assign', 'Const', 'RestoreV2', 'SaveV2', 'PlaceholderWithDefault', 'IsVariableInitialized', 'Placeholder', 'Identity','Switch','ReadVariableOp','VarIsInitializedOp','AssignAddVariableOp','ResourceApplyAdam','Pow','Cast','Add']

sess = None
if is_saved_tf_model or is_pb_file or is_saved_keras_model:
    ...
    with tf.Session() as sess:
        if is_saved_tf_model:
             saver = tf.train.import_meta_graph('/tmp/model/my_Firstmeta_test_model_mnist_dense.meta-15.meta')
             saver.restore(sess, tf.train.latest_checkpoint('/tmp/model/'))
        elif is_saved_keras_model:
             set_session(sess)
             #tf.keras.backend.set_learning_phase(0)
             reconstructed_model = tf.keras.models.load_model(netname)
             reconstructed_model.summary()

        else:
            new_model = tf.keras.models.load_model(netname)
            new_model.summary()
        ...
        while ops[last_layer_index].type in non_layer_operation_types:
            #print("--------------",ops[last_layer_index].name)
            last_layer_index -= 1
        #new_model = sess.graph.get_tensor_by_name(ops[last_layer_index].name + ':0')
        #new_model = sess.graph.get_tensor_by_name('Act4/Relu:0') 
        #new_model = sess.graph.get_tensor_by_name('Act4/Sigmoid:0')
        eran = ERAN(new_model, sess)
    
# here the pb or meta model is loaded with ERAN ****************************************************************
else:
    ...
    if is_onnx:
        ...
        print("''''''''''''''''''''''''''''pytorch model test ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''")
        print('**here the .tf model is processed with ERAN **')
    if domain == 'gpupoly' or domain == 'refinegpupoly':
        if is_onnx:
            translator = ONNXTranslator(model, True)
        else:
            print('**here the .tf model is translated with ERAN **')
        ...
        print('**here the .tf model is loaded with ERAN **')   
        eran = ERAN(model, is_onnx=is_onnx)
        

if not is_trained_with_pytorch:
    print("***************************************our case not is_trained_with_pytorch:")
...
if dataset=='acasxu':
    ...
    for i, test in enumerate(tests):
        ...
        print ("****************************our case for tests enumerate config mpeg7")
        print (i)
        ...
        if domain == 'gpupoly' or domain == 'refinegpupoly':
            ...
            print ("****************************our case for tests domaine")
            print (i)
        ...
        if is_correctly_classified == True:
            ...
            if config.normalized_region==True:
                print('perturbation region')

                # Apply the Deplacement3D or rotate3D_contour_bs function to the 'image' (contours with 120 coordinates) with specified parameters
                # The function returns Lower_bound_final and Upper_bound_final representing the abstract domain of the perturbed contour

                specLB, specUB = np.float64(Deplacement3D_contour_bs(image,0,0,0,250,0,0,100))
                #specLB, specUB = np.float64(rotate3D_contour_bs(image,4,5,0,0,0,0,10))
                specUB =specUB.reshape(1,240)
                specLB=specLB.reshape(1,240)
            ...
            if domain == 'gpupoly' or domain =='refinegpupoly':
                ...
                if domain.endswith("poly"):
                    print ("****************************our case for tests poly")
                    print (i)
                    #print ('perturbet mpeg7')
